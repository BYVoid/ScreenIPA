...
namespace HookEx {
    internal class KeyboardButton : Button {
        ...
        public KeyboardButton()
            ...
            base.Font = KeyboardButton.GetFont();
                //new Font("IPAPANNEW", 20, FontStyle.Regular, GraphicsUnit.Pixel, ((byte)(0)));
        ...
        private static Font GetFont()
        {
            ...
            {
                string[] fonts = { "Charis SIL", "Doulos SIL", "IPAPANNEW" };
                foreach (string font_name in fonts)
                {
                    default_font = new Font(font_name, 20, FontStyle.Regular, GraphicsUnit.Pixel, ((byte)(0)));
                    if (default_font.Name != SystemFonts.DefaultFont.Name && default_font.Name != FontFamily.GenericSansSerif.Name)
                        break;
                }
                if (default_font.Name == SystemFonts.DefaultFont.Name || default_font.Name == FontFamily.GenericSansSerif.Name)
                {
                    MessageBox.Show("您缺乏顯示國際音標所需字體，部分符號可能無法正常顯示。", "Screen IPA", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                }
            }
            return default_font;
        }
            ...
            if (this.ClientSize.Width > 3 && this.ClientSize.Height > 3) {
                ...
                using (Brush brush = new SolidBrush(base.ForeColor)) {
                    if (rect.Width > 4 && rect.Height > 2) {
                        rect.Inflate(-4, -2);
                        g.DrawString(base.Text, base.Font, (this.Enabled ? brush : Brushes.Gray), 12, 12, format);
                    }
                }
            }
        }
